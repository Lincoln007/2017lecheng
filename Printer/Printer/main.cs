...
namespace Printer
{
    public partial class main : Form
    {
        ...
        Queue<busi_printwork> Queueyamato = new Queue<busi_printwork>(); //yamato打印队列
        ...
        private void Timerpackge_Tick(object Sender, EventArgs e)
        {
            ...
            if(list.Count>0)//有数据要打印
            {
                ...
                {
                    if(!Queuepackge.Contains(item))
                    {
                        Queuepackge.Enqueue(item);
                    }
                }
            }
            if(Queuepackge.Count>0)//队列中有数据
            {
                try
                {
                    busi_printwork mypackge = Queuepackge.Peek(); //取出队列中对象
                    //打印包裹
        ...
        private void Timerzhuanyun_Tick(object Sender, EventArgs e)
        {
            ...
            if (list.Count > 0)//有数据要打印
            {
                foreach (var item in list)
                {
                    if (!Queuezhuanyun.Contains(item))
                    {
                        Queuezhuanyun.Enqueue(item);
                    }
                }
            }
            if (Queuezhuanyun.Count > 0)//队列中有数据
            {
                try
                {
                    busi_printwork mypackge = Queuezhuanyun.Peek(); //取出队列中对象
        ...
        private void Timeryamato_Tick(object Sender, EventArgs e)
        {
            ...
            if (list.Count > 0)//有数据要打印
            {
                foreach (var item in list)
                {
                    if (!Queueyamato.Contains(item))
                    {
                        Queueyamato.Enqueue(item);
                    }
                }
            }
            if (Queueyamato.Count > 0)//队列中有数据
            {
                try
                {
                    busi_printwork mypackge = Queueyamato.Peek(); //取出队列中对象
                    //打印yamato
