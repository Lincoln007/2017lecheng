...
{
    public class PrintYamato
    {
        public List<Myamato> mylist = null;
        ...
        /// <summary>
        /// 用yamato的发货方式打印Upacket的发货方式
        /// </summary>
        public void PrintUpacket2CM(bool p_ShowPrintDialog)
        {
            try
            {
                //1.获取打印机插件是否设置了打印拣选单的面单
                ...
                dt.Rows.Add(row);
                PrintCenter printer = new PrintCenter();
                printer.ZPLPrintUpacket2CM(dt, true, 1);
            }
            catch (Exception ex)
            {
                throw ex;
